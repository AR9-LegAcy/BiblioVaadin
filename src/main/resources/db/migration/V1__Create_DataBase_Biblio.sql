...
CREATE TABLE biblio.LIVRE (
   ...
   CREATED_AT_LIVRE     TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
   UPDATED_AT_LIVRE     TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT PK_LIVRE PRIMARY KEY (ID_DOCUMENT),
    CONSTRAINT CK_LIVRE_PAGES CHECK (NB_PAGES > 0)
);

/* Index : LIVRE_PK */
CREATE UNIQUE INDEX LIVRE_PK ON biblio.LIVRE (ID_DOCUMENT);
...
CREATE TABLE biblio.ECRIRE (
    ...
    CONSTRAINT PK_ECRIRE PRIMARY KEY (ID_DOCUMENT, ID_AUTEUR)
);

/* Index : ECRIRE_PK */
CREATE UNIQUE INDEX ECRIRE_PK ON biblio.ECRIRE (ID_DOCUMENT, ID_AUTEUR);

/* Index FK : ECRIRE_LIVRE_FK */
CREATE INDEX ECRIRE_LIVRE_FK ON biblio.ECRIRE (ID_DOCUMENT);
...
FOREIGN KEY (ID_DOCUMENT)
REFERENCES biblio.LIVRE (ID_DOCUMENT)
ON DELETE RESTRICT ON UPDATE RESTRICT;
