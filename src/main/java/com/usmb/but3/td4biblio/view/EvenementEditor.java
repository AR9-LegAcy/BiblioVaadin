...
public class EvenementEditor extends VerticalLayout implements KeyNotifier {
    ...
    ComboBox<TypeEvenement> typeEvenement =
            new ComboBox<>("Type d'évènement");

    Button save =
            new Button("Sauvegarder", VaadinIcon.CHECK.create());

    Button cancel = new Button("Annuler");

    Button delete =
            new Button("Supprimer", VaadinIcon.TRASH.create());

    HorizontalLayout actions =
            new HorizontalLayout(save, cancel, delete);

    Binder<Evenement> binder =
            new Binder<>(Evenement.class);

    private ChangeHandler changeHandler;

    public EvenementEditor(
        ...
        typeEvenement.setItemLabelGenerator(
        ...
        cancel.addClickListener(e -> editEvenement(evenement));
    ...
    void delete() {
        evenementService.deleteEvenementById(
                evenement.getId());
        changeHandler.onChange();
    }
    ...
    public final void editEvenement(Evenement e) {
        ...
        if (persisted) {
            evenement =
                    evenementService.getEvenementById(e.getId());
        } else {
            evenement = e;
        }
